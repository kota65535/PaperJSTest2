...
export class Rail implements Storable<RailStoreState> {
   ...
   */
  enableFeederSockets() {
    this.feederSockets.forEach(f => f.enabled = true)
  }

  /**
   * フィーダーの検出を無効化する。
   */
  disableFeederSockets() {
    this.feederSockets.forEach(f => f.enabled = false)
  }
   ...
   */
  enableGapSockets() {
    this.gapSockets.forEach(g => g.enabled = true)
  }
   ...
   */
  disableGapSockets() {
    this.gapSockets.forEach(g => g.enabled = false)
  }
